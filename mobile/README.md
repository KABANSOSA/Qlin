...
Релиз и разработка ведутся **только из каталога `mobile/`**. Папка `mobile-cleaner/` в репозитории не используется для продакшена.
...
│   ├── customer/           # клиент: вкладки (tabs), новый заказ (new), карточка заказа
│   └── cleaner/            # клинер: вкладки (tabs), карточка заказа
├── src/
